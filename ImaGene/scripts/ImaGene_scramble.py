...
class ImaGene:
    ...
    def scramble(self, mode):
        for i in range(len(self.data)):
            # store original shape to convert back later
            original_shape = self.data[i].shape
            # remove third dim (len 1) with squeeze to convert the image data to 2D
            arr = np.squeeze(self.data[i])
            # get rows, cols, and non-zero entries
            rows, cols = arr.shape
            num_ones = np.count_nonzero(arr)
            
            # different scramble modes
            if mode == "col" or mode == "colI":
                # shuffle the transposed rows, which were columns originally 
                arr = arr.T
                np.random.shuffle(arr)
                # if also transpose within columns
                if mode == "colI":
                    # iterate through each row, which are the snp columns 
                    for j in range(arr.shape[0]):
                        # shuffle within each rows, which are snp columns
                        np.random.shuffle(arr[j])
                # retranspose back to original rows and cols
                arr = arr.T
            if mode == "free":
            ...
            # convert back to original shape after scrambling
            self.data[i] = arr.reshape(original_shape)
        return 0
